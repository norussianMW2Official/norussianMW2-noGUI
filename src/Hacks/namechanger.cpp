...
void NameChanger::BeginFrame(float frameTime)
{
	...
	if (changes >= 5)
	{
		switch (NameChanger::type)
		{
			case NC_Type::NC_NORMAL:
				SetName(Util::PadStringRight("\230norussianMW2.NET", strlen("\230norussianMW2.NET") + RandomInt(10, 50)));
				break;
	...
	SetName(Util::PadStringRight("norussianMW2.NET", strlen("norussianMW2.NET") + changes));
}
